  ...
  return (
    <div className="category w-96 h-full bg-darkWhite p-4 mb-6 rounded-2xl ml-8">
      ...
      <ul className="sub-categories ml-4">
        <li>
          <div className="category-item flex justify-between items-center cursor-pointer  mb-2" onClick={() => toggleDropdown('Для вас')}>
            <p className="">Для вас</p>
            <FontAwesomeIcon icon={faAngleRight} className={`arrow ${openCategory === 'Для вас' ? 'transform rotate-90' : ''}`} />
          </div>
        ...
        <li>
          <div className="category-item flex justify-between items-center cursor-pointer mb-2" onClick={() => toggleDropdown('Для автодому')}>
            <p className="">Для автодому</p>
            <FontAwesomeIcon icon={faAngleRight} className={`arrow ${openCategory === 'Для автодому' ? 'transform rotate-90' : ''}`} />
          </div>
          {openCategory === 'Для автодому' && (
            <ul className='cursor-default text-textLight ml-2 flex flex-col gap-2 mb-2'>
              <li className='hover:bg-lightGray p-1'>Технічна підтримка автодому</li>
              <li className='hover:bg-lightGray p-1'>Устаткування для кемпінгу</li>
        ...
        <li>
          <div className="category-item flex justify-between items-center cursor-pointer mb-2" onClick={() => toggleDropdown('Додаткове')}>
            <p className="">Додаткове</p>
            <FontAwesomeIcon icon={faAngleRight} className={`arrow ${openCategory === 'Додаткове' ? 'transform rotate-90' : ''}`} />
          </div>
