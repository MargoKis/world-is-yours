...
const CartItem = ({ product, handleQuantityChange, handleRemoveItem }) => {
  ...
  return (
    <div className='flex flex-row justify-between items-end border-b border-gray p-4'>
      ...
      <div>
        <img src={trash} alt='delete icon' className='cursor-pointer w-10 h-10 mb-9 p-2 border border-transparent rounded-lg duration-100 hover:border-slate-400 focus:border-slate-400' onClick={() => handleRemoveItem(product.basketId)} tabIndex='0' />
      </div>
    </div>
  );
};
